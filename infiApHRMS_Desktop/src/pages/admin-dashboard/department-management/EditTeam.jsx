...
const EditTeam = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      await Promise.all([fetchDepartments(), fetchEmployees({ limit: 1000 })]);
      const team = teams.find(t => t.id === id);
      if (team) {
        setFormData({
          name: team.name || '',
          department: team.departmentId || '',
          lead: team.leadId || team.lead || '',
      ...
      <div className="w-full max-w-[800px] bg-white rounded-[48px] border border-slate-50 shadow-soft overflow-hidden">
        <div className="p-12 md:p-16">
          ...
          <div className="flex items-center gap-8 mb-16 px-2">
            <div className="w-20 h-20 bg-indigo-600 text-white rounded-[28px] flex items-center justify-center shadow-2xl shadow-indigo-100 transition-transform hover:scale-110">
              <Users size={36} />
            </div>
            <div>
              <h1 className="text-4xl font-black text-slate-800 tracking-tight leading-none mb-2">Edit Team</h1>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1 leading-none text-left">Update team details and objectives</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="flex flex-col md:flex-row items-center gap-6 pt-10">
              <button 
                type="submit"
                className="w-full md:flex-1 py-6 bg-slate-900 text-white rounded-[24px] font-black text-[10px] uppercase tracking-[0.25em] shadow-2xl shadow-indigo-100 hover:shadow-indigo-300 hover:-translate-y-1 transition-all active:scale-95 flex items-center justify-center gap-4"
              >

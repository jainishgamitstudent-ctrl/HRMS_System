...
import { getHrProfile, updateProfile, verifyEmployeeProfileUpdate } from '../../../services/hrApi';
import {
  ArrowLeft,
  Save,
  X,
...
const EditProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await updateProfile(userId, profileData, profilePicture);

      if (res?.data?.otpRequired || res?.otpRequired) {
        setShowOtpModal(true);
        if (res?.data?.devOtp || res?.devOtp) setDevOtp(res?.data?.devOtp || res?.devOtp);
        setSaving(false);
        return;
      }
  ...
  const handleVerifyAndSave = async () => {
    ...
    try {
      const userId = user?._id || user?.id;
      const res = await verifyEmployeeProfileUpdate(userId, otpString);
